...
class Wiggle:
        ...
        with open(self.file_path, "r") as raw_file:
            ...
            for content_header, content in all_contents.items():
                ...
                if chrom_size is None:
                    ...
                    print(f"Could not get the chromosome size from Internet, "
                          f"maximum wiggle position ({chrom_size}) will be used")
                else:
                    print(f'====> Size retrieved for {seqid} is: {chrom_size}')
                self.chrom_sizes[seqid] = chrom_size
                self.spans[seqid] = current_wiggle_meta["variableStep_span"]
                self.orientations[seqid] = "-" if np.any(self.coverages[seqid][:, 1] < 0) else "+"
            s = '\n     └── '
            print(f"===> Parsed condition: {self.track_name}\n"
            ...
            if len(empty_seqids) > 0:
                print(f"     + Sequence IDs not included:\n"
    ...
    def generate_1d_signal(self):
        for k in self.coverages.keys():
            ...
            self.signals[k] = \
                np.abs(pd.merge(df1, df2, right_on="loc", left_on="loc", how="outer").fillna(0.0)["score"].to_numpy())
    ...
    def _parse_wiggle_str(in_str):
        ...
        for i in range(0, len(content_list), 2):
            if content_list[i + 1].isspace():
                empty_seqids.append(content_list[i].split('chrom=')[-1].split(' ')[0].replace('\"', ''))
                continue
    ...
    def get_seq_length(seq_id) -> int or None:
        ...
        try:
            print(f"===> Trying to get sequence ID length for: {seq_id}")
            ...
            print(f"===> Error fetching metadata: {e.args}")
            return None
        seq_len = int(str(esummary_record['Length']).replace("IntegerElement(", "").split(",")[0])
        return seq_len
